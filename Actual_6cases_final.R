...
details <- setNames(details, c("probability", paste0("Tumour Pair", 1:49)))
...
details[, 27] <- c(rep(0, 1),rep(1, 3), rep(0, 3),rep(2, 1), rep(0, nrow(details) - 8))
...
details[, 27] <- c(rep(0, 4),rep(1, 2),rep(0, 1),rep(2, 1), rep(0, nrow(details) - 8))
...
init.para <- c(-1, 1, 0.1)  # You may need to adjust these initial values based on your data and prior knowledge.
...
result <- clonEM(mutmat, init.para, xigrid, conv.crit, niter)
...
cat("Estimated proportion of clonality Actual 6 cases =",result$para[3])


##### Best estimate under Actual 6 cases   is 0.05084
